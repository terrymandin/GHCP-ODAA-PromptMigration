...
SOURCE_SSH_KEY_RAW="${SOURCE_SSH_KEY:-~/.ssh/<source_key>.pem}"
...
DISCOVERY_DIR="$STEP2_DIR/Discovery/source"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
RAW_OUT="$DISCOVERY_DIR/source-discovery-$TIMESTAMP.raw.txt"
REPORT_MD="$DISCOVERY_DIR/source-discovery-$TIMESTAMP.md"
REPORT_JSON="$DISCOVERY_DIR/source-discovery-$TIMESTAMP.json"
...
if [[ -n "$SOURCE_SSH_KEY" && ! -r "$SOURCE_SSH_KEY" ]]; then
  OVERALL_STATUS="FAIL"
  NOTES="SSH key file is not readable: $SOURCE_SSH_KEY"
fi
...
if [[ "$OVERALL_STATUS" != "FAIL" ]]; then
  ...
  if [[ -n "$SOURCE_SSH_KEY" ]]; then
    SSH_CMD+=( -i "$SOURCE_SSH_KEY" )
  fi
  SSH_CMD+=("${SOURCE_SSH_USER}@${SOURCE_HOST}" "bash -s -- '$ORACLE_USER' '$SOURCE_REMOTE_ORACLE_HOME' '$SOURCE_ORACLE_SID' '$SOURCE_DATABASE_UNIQUE_NAME'")
...
{
  echo "# Source Discovery Report"
  echo
  echo "- Timestamp: $TIMESTAMP"
  echo "- Status: $OVERALL_STATUS"
  echo "- Source host: $SOURCE_HOST"
  echo "- SSH user: $SOURCE_SSH_USER"
...

{
  ...
  echo "  \"ssh_user\": \"$(json_escape "$SOURCE_SSH_USER")\"," 
...
echo "Source markdown report: $REPORT_MD"
echo "Source json report: $REPORT_JSON"
