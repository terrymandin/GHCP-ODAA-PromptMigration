...
| Field | Value |
|-------|-------|
| **Generated** | Thu Mar 13 00:00:00 UTC 2026 |
| **Run by** | zdmuser@zdm-server |
| **Overall result** | **PASS** |
...
| Result | **✅ PASS** |
| Remote Hostname | `source-db-node1` |
...
| SSH Key | `~/.ssh/odaa.pem` |
| Result | **✅ PASS** |
| Remote Hostname | `target-odaa-node1` |
...
4. **Host key verification** — If `known_hosts` is causing issues, run:
   ...
   ```
5. **Permission denied** — Confirm the correct user (`azureuser` / `opc`) is listed in `~/.ssh/authorized_keys` on each host.
