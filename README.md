...
- **Pro tip**: Step 2 installs/verifies ZDM 26.1 and writes `Artifacts/Phase10-Migration/Step2/zdm-install-report.md`.
- **Pro tip**: Step 3 collects SSH configuration interactively and writes `Artifacts/Phase10-Migration/Step3/ssh-config.md`. Pre-populate this file to skip interactive prompting during testing.
- **Pro tip**: Step 4 collects database variables interactively and writes `Artifacts/Phase10-Migration/Step4/db-config.md`. Pre-populate this file to skip interactive prompting during testing.
...
- **`.github/prompts/`**: Copilot prompt files for each migration phase — invoke with `@PromptName` in Copilot Chat
  ...
  - `Phase10-Step2-Install-ZDM.prompt.md` — optionally create ZDM Azure VM; install/verify ZDM 26.1
  - `Phase10-Step3-Configure-SSH-Connectivity.prompt.md` — SSH connectivity between source, target, and ZDM server
  - `Phase10-Step4-Generate-Discovery-Scripts.prompt.md` — generate and run database discovery scripts
  - `Phase10-Step5-Discovery-Questionnaire.prompt.md` — analyze discovery output and complete migration decisions
  - `Phase10-Step6-Fix-Issues.prompt.md` — resolve ZDM prerequisite check failures
  - `Phase10-Step7-Generate-Migration-Artifacts.prompt.md` — generate ZDM response file and migration runbook
...
- **`Artifacts/`**: Generated output from running prompts (git-ignored content)
  - **`Artifacts/Phase10-Migration/Step3/ssh-config.md`** — SSH connectivity config written interactively by Step 3 (pre-populate to bypass interactive collection)
  - **`Artifacts/Phase10-Migration/Step4/db-config.md`** — database and ZDM config written interactively by Step 4 (pre-populate to bypass interactive collection)
...
Use the migration tool to migrate to Azure. The ZDM-based workflow (Steps 1–7) is fully implemented via Copilot prompt files. Step 1 sets up Remote-SSH, Step 2 installs/verifies ZDM 26.1, Steps 3–6 configure and validate migration readiness, and Step 7 generates migration artifacts.
...
2. Install [GitHub Copilot](https://copilot.github.com/) and use Auto model selection
...
| Step | Run In | Purpose |
|------|--------|---------|
| `@Phase10-Step3-Configure-SSH-Connectivity` | VS Code | Validate SSH connectivity from jumpbox to source/target |
| `@Phase10-Step4-Generate-Discovery-Scripts` | VS Code | Generate and run database discovery |
| Run discovery scripts | ZDM Server | Collect source/target/ZDM facts |
| `@Phase10-Step5-Discovery-Questionnaire` | VS Code | Analyze results, create migration plan |
| `@Phase10-Step6-Fix-Issues` | VS Code + ZDM | Resolve blockers iteratively |
| `@Phase10-Step7-Generate-Migration-Artifacts` | VS Code | Generate RSP, runbook, and migration commands |
