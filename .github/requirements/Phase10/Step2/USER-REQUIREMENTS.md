# Step2 User Requirements - ZDM Installation Verification
...
> **Note:** The ZDM Azure VM is created during Step 1 (VM Create + Remote-SSH Setup). By the time Step2 runs, the VM already exists and the Remote-SSH connection is established.

**Execution model**: Step2 runs entirely in the Remote-SSH VS Code session on the jumpbox, logged in as `zdmuser`. The `zdmuser` OS account, its home directory, the ZDM prerequisite packages, and the `/u01/app/zdm*` directories are all set up during Step 1; this step should verify they exist before proceeding.
...
Step2 writes one artifact using file tools after verification or installation completes:
