...
| SQL\*Net direct reachability: source → target SCAN:1521 | `nc -zv $TARGET_SCAN_ADDR 1521` from source host | **If connection succeeds**: direct SQL\*Net is available — do NOT set `TGT_SSH_TUNNEL_PORT` in the RSP (including it when direct connectivity works causes `localhost:<port>` precheck failures). **If connection fails**: set up SSH tunnel AND add `TGT_SSH_TUNNEL_PORT` to RSP; do not add it otherwise. | BLOCKER | 4.1 Prerequisites for Online Physical Migration |
