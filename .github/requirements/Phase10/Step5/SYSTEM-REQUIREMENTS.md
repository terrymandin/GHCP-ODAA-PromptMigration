...
## S5-13: Pre-execution risk banner and acknowledgment gate

Before displaying the S5-11 execution menu, the prompt must always display the following risk banner. The banner is mandatory — it must not be skipped or abbreviated.
...
Behavior rules:
1. Always show the banner before the S5-11 menu, even on subsequent iterations of Step5.
2. If no `ORACLE-HOME` or `OS` scope scripts are present, omit the blast-radius paragraph but keep the rest of the banner.
3. Do not display the S5-11 execution options until the user types `CONFIRM`.
...
## S5-09: Default no-execution — conditional execution on explicit user request

1. Remediation scripts and the verification script are **generated and saved to disk only** by default.
2. The prompt must **not execute** any remediation or verification script unless the user explicitly requests execution after seeing the S5-11 script inventory and choice menu.
...
5. Never execute a script silently or without prior display of the S5-11 menu.
