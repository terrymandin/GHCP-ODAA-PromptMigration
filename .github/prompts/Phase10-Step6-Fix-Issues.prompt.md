...
**Scripts are generated and saved to disk by default** (S6-09). Execution is the operator's responsibility after reviewing the generated artifacts. Conditional inline execution is available after the script inventory and risk banner are presented — see Part 5. Non-remediable issues (for example source database role/open-mode transitions needed to reach `PRIMARY` + `READ WRITE`) are documented as manual actions in `Issue-Resolution-Log.md` and do not receive autogenerated fix scripts.
