...
Optional acceleration: when an Oracle MCP server is configured and direct DB connectivity is permitted, use Oracle MCP for selected read-only SQL evidence collection. SSH plus SQL*Plus remains the canonical baseline and fallback.
...
- If Oracle MCP is used, keep operations read-only only (metadata + SELECT). Do not perform DDL, DML, parameter changes, service restarts, or host-level changes via MCP.
...
> `SOURCE_GI_TYPE`, `TGT_REDODG`, and `TGT_RECODG` are written by Copilot after discovery (Phases 8 and 9). Write the file before discovery with these fields blank; update it after each discovery phase completes.
...
1. **Determine ZDM version** from Phase 7 (`$ZDM_HOME/bin/zdmcli -version` was run as part of ZDM server discovery). Extract the version string (e.g., `26.1`). Use `26.1` as the default if the version could not be determined.

2. **Determine migration method**: read `MIGRATION_METHOD` from `db-config.md` if present; default to `ONLINE_PHYSICAL` if not yet confirmed.

3. **Select catalog file path** based on migration method:
...
4. **Read catalog**: Use `read_file` on the selected path. If the version directory does not exist, substitute `26.1` and log a version mismatch warning:
   > `Prerequisite catalog — WARNING: version <discovered> not found, using 26.1`

5. **Use catalog**: The loaded catalog file is the authoritative Layer 0, Layer 1, and Layer 2 check list for source and target discovery (used in Phase 8 items 6 and 15, and Phase 9).
...
For SQL-only source evidence items, Oracle MCP may be used first when available. If Oracle MCP returns incomplete data or fails, retry the item with SSH+SQL*Plus and continue.
...
For SQL-only target evidence items, Oracle MCP may be used first when available. If Oracle MCP returns incomplete data or fails, retry the item with SSH+SQL*Plus and continue.
...
For each discovery item captured in reports, include the evidence source marker: `ssh/sqlplus` or `oracle-mcp`.
...
## Variable Learn-More Reference

When the user types `?`, `??`, or `help` in response to any variable prompt, display the entry from this table, then re-present the original prompt (CR-15-B, CR-15-C).

| Variable | Purpose | How to find | Constraints / pitfalls | Example |
|----------|---------|-------------|------------------------|---------|
| `SOURCE_REMOTE_ORACLE_HOME` | Path to Oracle software home on the source host — ZDM uses it to locate `sqlplus`, `lsnrctl`, and `OPatch` | `cat /etc/oratab` on source; look for lines matching the target SID | Must be the ORACLE_HOME directory, not `$ORACLE_BASE`; verify it exists: `ssh ... test -d <path>` | `/u01/app/oracle/product/19.0.0/dbhome_1` |
...
| `SOURCE_DATABASE_UNIQUE_NAME` | `DB_UNIQUE_NAME` of the source database — used as the Data Guard primary name and in `-sourcedb` when GI is active | `SELECT db_unique_name FROM v$database;` on source; or `srvctl config database -d <name>` | Must match the value in the source controlfile; must differ from `TARGET_DATABASE_UNIQUE_NAME` | `ORCL_PHX` |
...
16. **Grid Infrastructure detection** (S4-06 item 16): run the following on the source host:
   ...
   ```
   - If CRS returns a version string AND the database is registered with srvctl → set `SOURCE_GI_TYPE=grid`.
   - Otherwise → set `SOURCE_GI_TYPE=standalone`.
   - Write the resolved value to `db-config.md` after source discovery completes. **Using the wrong value causes PRGZ-3928** when ZDM calls the `zdmcli migrate database` command with the incorrect `-sourcesid` vs `-sourcedb` flag.
...
EOF'"
   ```
   Or if the grid user has `asmcmd`:
   ```bash
   ssh ... "asmcmd lsdg --discovery 2>/dev/null"
   ```
   - Identify the disk group used for **redo logs** → write to `db-config.md` as `TGT_REDODG`.
   - Identify the disk group used for **recovery/FRA** → write to `db-config.md` as `TGT_RECODG`.
   - If PLATFORM_TYPE is `EXACS` or `EXACC` and either value is blank, warn the user: omitting these values causes **PRCG-1054** at ZDM evaluation time.
