...
- **Execution model**: Steps 2–7 run inside a VS Code Remote-SSH session connected to the ZDM jumpbox as `zdmuser`. Step 1 is the exception — it runs in the local VS Code PowerShell terminal to set up that connection.
- **Config artifacts** — `Artifacts/Phase10-Migration/Step3/ssh-config.md` and `Artifacts/Phase10-Migration/Step4/db-config.md` — are the runtime source of truth for SSH and database variables. Generated scripts must not read or parse these files at runtime.
...
- SSH variables (hosts, users, key paths) live in `Artifacts/Phase10-Migration/Step3/ssh-config.md`.
- DB and ZDM variables (`SOURCE_REMOTE_ORACLE_HOME`, `SOURCE_ORACLE_SID`, `TARGET_REMOTE_ORACLE_HOME`, `TARGET_ORACLE_SID`, `SOURCE_DATABASE_UNIQUE_NAME`, `TARGET_DATABASE_UNIQUE_NAME`, `ZDM_HOME`) live in `Artifacts/Phase10-Migration/Step4/db-config.md`.
